...
const GroupLvl = ({stickerH4, aboutTextPoppins, skills}: GroupLvlPropsType) => {
  const h4Styles = {backgroundColor:stickerH4.stripBgColor?COLORS[stickerH4.stripBgColor]:COLORS.basicOrange,color:stickerH4.stripTextColor?COLORS[stickerH4.stripTextColor]:COLORS.white}
  ...
  if(skills.length>0){
    return (
      <div className={styles.container} onClick={showSkillsHandler}>
        {!showSkills&&<>
        <h4 style={h4Styles}>{stickerH4.title} {stickerH4.titleStrip}</h4>
        <h5>{aboutTextPoppins.title}</h5>
        <p>
         {aboutTextPoppins.description}
        </p>
        
        </>}
        {showSkills&&<ul>{skills.map((skill,index)=><li key={index}>{skill}</li>)}</ul>}
        <span className={styles.button}>{!showSkills?'Pokaż umiejętności':'Pokaż opis'}</span>
      </div>
    )
  }
  if(skills.length===0){
